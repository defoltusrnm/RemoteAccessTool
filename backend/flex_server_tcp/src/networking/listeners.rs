use anyhow::Context;
use flex_net_core::networking::address_src::EndpointAddress;
...
impl NetListener for NetTcpListener {
    async fn bind(addr: EndpointAddress) -> Result<Self, anyhow::Error> {
...
impl NetAcceptable<NetTcpConnection> for NetTcpListener {
    async fn accept(&self) -> Result<NetTcpConnection, anyhow::Error> {
        let (socket, _) = self
            .inner_listener
            .accept()
            .await
